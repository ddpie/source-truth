...
| `push-local-repo.sh` | p1 | 已实现 | **客户机侧** local 仓上传：`rsync` over SSH 把本地仓推到索引主机暂存目录 `/data/repo/<subdir>.incoming`，再经 `sudo bash reindex_local_repo.sh <subdir>` 在主机本地原地同步到 live 副本——常规推送由 watcher 增量重建索引、按变更清单增量刷新术语表、不停 bridge（与 git pull 同路径）；首次推送停 bridge 全量建一次图。用于 `projects.json` 里 `source:"local"` 的仓（无 git remote），重跑即刷新。安全：subdir 正则校验、`--safe-links --no-links` 防软链逃逸、不支持自由 `--ssh-opts`（只认 `--identity`）。`--dry-run` 只打印命令。 |
