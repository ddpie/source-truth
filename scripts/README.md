...
| `teardown.sh` | p1 | 已实现 | **有序销毁**：按反依赖顺序删 deploy-all.sh 创建的资源（runtime → EC2 → NAT+EIP → DNS → 子网/路由/IGW/SG → VPC → ECR），资源从 `.local/deploy-config` 读；读不到则按 `source-truth-*` Name tag 发现（覆盖中途失败的残留）。`--dry-run` 只打印计划；默认交互确认，`--yes` 跳过；IAM 角色 + S3 桶（账号共享）默认保留，`--include-shared` 才删。破坏性操作，见 AGENTS.md「ask first」。 |
