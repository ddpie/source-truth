...
| `trace.sh` | p1 | 已实现 | 按 traceId 合并查询网关 + agent microVM 两个 log group 的全链路时间线（`--since-hours` 扩窗、`--raw` 不合并）。排错用：定位一次问答卡在网关、invoke 还是 agent 侧。 |
