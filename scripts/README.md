...
| `launch-host.sh` | p1 | 已实现 | `--local`（单台 EC2）模式入口，在**运维本地**跑：选 profile → 建 IAM（`create-iam.sh`）→ 自动建 source-truth 专用网络（VPC/公私子网/IGW/NAT，复用 `provision_network.sh`）+ host 安全组（只放行运维 IP 的 22）→ 在公有子网起一台 ARM64 EC2 挂好实例角色 → 打印进 EC2 的部署命令。已有 host 默认复用（`--new-host` 强制新建），`--dry-run` 只打印计划，选择记入 `.local/launch-host.<account>.env` 供重跑预填。 |
