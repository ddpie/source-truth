...
| `push-local-repo.sh` | p1 | 已实现 | **客户机侧** local 仓上传：`rsync` over SSH 把本地仓推到索引主机暂存目录 `/data/repo/<subdir>.incoming`，再经 `sudo bash reindex_local_repo.sh <subdir>` 触发主机侧停 bridge→切换→重建→起 bridge（失败原子回滚）。用于 `projects.json` 里 `source:"local"` 的仓（无 git remote），重跑即刷新。安全：subdir 正则校验、`--safe-links --no-links` 防软链逃逸、不支持自由 `--ssh-opts`（只认 `--identity`）。`--dry-run` 只打印命令。 |
