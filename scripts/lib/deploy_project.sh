...
validate_projects_config "$PROJECTS_CFG" "$SCRIPT_DIR" || exit 2
...
read_proj() { python3 - "$PROJECTS_CFG" "$PID" "$ROOT/agent-container" "${DEPLOY_MODEL:-}" <<'PY'
import json, sys, shlex
sys.path.insert(0, sys.argv[3])
from agent_settings import project_settings
...
settings = project_settings(p, schema_version=cfg.get("schemaVersion", 1), legacy_model=sys.argv[4])
for key, value in {"AGENT_SDK": settings.sdk, "QA_MODEL": settings.model,
                   "GLOSSARY_MODEL": settings.glossary_model, "AGENT_MAX_TURNS": settings.max_turns}.items():
...
# Validate all locally knowable runtime inputs before the first host-side write.
python3 "$SCRIPT_DIR/deploy_runtime.py" --validate-only \
  --region "$REGION" --account "$ACCOUNT" --role-arn "$ROLE_ARN" --image "$ECR_URI" \
  --sdk "$AGENT_SDK" --model "$QA_MODEL" --agent-max-turns "$AGENT_MAX_TURNS" \
...
GL_RC=0
if [[ "$GLOSSARY_MODEL" == "$QA_MODEL" ]]; then
  GL_MODEL="$RT_MODEL"
else
  GL_MODEL="$(resolve_model_for_region "$GLOSSARY_MODEL" "$REGION")" || GL_RC=$?
fi
[[ "$GL_RC" == 0 ]] || say warn "[$PID] glossary profile '$GL_MODEL' remains unverified"
say info "[$PID] sdk=$AGENT_SDK answer=$RT_MODEL glossary=$GL_MODEL (Bedrock Runtime)"

# Glossary is opt-in (deploy-all.sh --with-glossary, persisted as DEPLOY_WITH_GLOSSARY). Off ⇒ hand
# activate_project.sh an EMPTY glossary model, which it treats as "engine disabled" (no initial cc
# build, pull-only refresh timer). The runtime's answer model (RT_MODEL) is unaffected.
# Upgrade path (reached directly from install.sh redeploy): a deploy-config with a host but no
# DEPLOY_WITH_GLOSSARY key predates the switch, when the glossary was always on. Keep it ON and
# persist, mirroring deploy-all.sh's resolve_optin; --with-glossary=false is the explicit off.
if [[ -z "${DEPLOY_WITH_GLOSSARY:-}" ]]; then
  say warn "现有部署早于开关 / existing deployment predates the opt-in switches — keeping glossary ON;"
  say warn "  pass --with-glossary=false to deploy-all.sh / install.sh (or set DEPLOY_WITH_GLOSSARY=false in .local/deploy-config) to turn it off"
  DEPLOY_WITH_GLOSSARY=true
  update_env "$CONFIG_FILE" DEPLOY_WITH_GLOSSARY true
fi
GLOSSARY_ENABLED="${DEPLOY_WITH_GLOSSARY:-false}"
[[ "$GLOSSARY_ENABLED" == true ]] || GL_MODEL=""
...
REMOTE_CMD="set -e
# The base re-bootstrap and every project publish write the same app tree.
# Serialize the whole activation, including its download and temporary files.
LOCK=\${REBOOT_LOCK_FILE:-/var/lock/source-truth-rebootstrap.lock}
command -v flock >/dev/null 2>&1 || { echo 'ACTIVATE_FAILED: flock is required for host deployment serialization' >&2; exit 1; }
exec 9>\"\$LOCK\"
flock -n 9 || { echo \"ACTIVATE_FAILED: another host deployment holds \$LOCK\" >&2; exit 75; }
...
for f in http_bridge.py activate_project.sh git_fetch.sh glossary_refresh.sh glossary_worker.sh reindex_local_repo.sh requirements.txt openai_backend.py bedrock_converse.py openai_glossary.py glossary_config.py glossary-requirements.lock setup_glossary.sh; do
...
PROJECT_ID='${PID}' GIT_SECRET_ID='${GIT_SECRET_ID}' AGENT_SDK='${AGENT_SDK}' MODEL='${GL_MODEL}' GLOSSARY_ENABLED='${GLOSSARY_ENABLED}' REPO_MANIFEST_JSON=\"\$(cat /tmp/manifest-${PID}.json)\" bash /opt/idx/app/activate_project.sh
...
RT_OUT="$(python3 "$SCRIPT_DIR/deploy_runtime.py" \
  ...
  --sdk "$AGENT_SDK" --agent-max-turns "$AGENT_MAX_TURNS" \
  --subnets "$SUBNET" --security-groups "$RUNTIME_SG" \
...
if ! aws secretsmanager describe-secret --region "$REGION" --secret-id source-truth/log-hash-salt >/dev/null 2>&1; then
  ...
  printf '%s' "$GW_SALT" | aws secretsmanager create-secret --region "$REGION" --name source-truth/log-hash-salt \
    --secret-string file:///dev/stdin --description 'source-truth gateway LOG_HASH_SALT (telemetry de-identification)' >/dev/null 2>&1 \
...
# probe_verdict <rc> <pid> : map e2e-probe.py's exit code to the operator verdict.
# 0 = answered correctly; any other result (including 2 = probe could not run) → this script
# exits 3 = "deployed, but the smoke probe failed" (deploy-all / install.sh report it as such, not
# as a failed deploy — the bridge, runtime and gateway ARE up).
probe_verdict() {
  case "$1" in
    0) say ok "部署完成且真实问答通过 / deployed and answered a real question ($2)" ;;
    2) say warn "验收探针无法运行（缺 boto3 或配置），部署本身已完成但未验收 / smoke could not run (rc 2); deployed but unverified"
       return 1 ;;
    *) say err "真实问答未通过 / the bot did not answer correctly (probe rc $1)."
       say err "  → 见 docs/runbook_zh.md §五 验证 / §八 排错 — see docs/runbook_en.md §5 Verify / §8 Troubleshoot"
       return 1 ;;
  esac
}
...
# The probe is skipped only when EVERY repo is local: a git repo is cloned + indexed by activate, so
# a mixed project can be probed right away.
...
' 2>/dev/null || true)"
ALL_LOCAL="$(REPO_MANIFEST_JSON="$REPO_MANIFEST_JSON" python3 -c '
import json,os
rs=json.loads(os.environ["REPO_MANIFEST_JSON"]).get("repos",[])
print("true" if rs and all(r.get("source")=="local" for r in rs) else "false")
' 2>/dev/null || echo false)"
if [[ -n "${LOCAL_SUBS// }" ]]; then
  say warn "项目 $PID 含本地仓 [${LOCAL_SUBS# }]：后端已就绪，但这些仓在推代码前无法作答（索引为空）。"
...
if [[ "$ALL_LOCAL" == true ]]; then
  say info "跳过真实问答验收（所有仓库均为本地仓，代码尚未推送）/ post-deploy smoke skipped until the local repos are pushed"
elif [[ "${NO_PROBE:-0}" == 1 || "${E2E_PROBE:-1}" == 0 ]]; then
  say info "跳过真实问答验收 / post-deploy smoke skipped (--no-probe / E2E_PROBE=0)"
else
  # POST-DEPLOY SMOKE: "fully deployed" above only proves the units are up. Ask a real
  # code question through the same invoke path the gateway uses, so a broken model/bridge/IAM shows up
  # here, not in the operator's first Feishu message.
  say step "真实问答验收 / post-deploy smoke: 1 code question via the runtime（模型费用 / model cost）"
  _PRC=0
  python3 "$ROOT/scripts/e2e-probe.py" --region "$REGION" --project "$PID" --smoke || _PRC=$?
  probe_verdict "$_PRC" "$PID" || exit 3   # 3 = deployed, smoke failed (see probe_verdict)
fi
