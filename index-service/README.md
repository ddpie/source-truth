...
独立常驻服务（非会话容器内）。部署流程：deploy 时打包仓库 tar.gz→S3 → `bootstrap.sh` 首次解包到
...
- **架构 / glibc**：会话与 index-service 主机是 **ARM aarch64**，且 `codegraph-server` 需
  **glibc ≥ 2.38**（因此基础系统用 Ubuntu 24.04 / glibc 2.39；Amazon Linux 2023 的 2.34 实测会崩溃）。
...
模块位于包根（**不在 `src/`**）：`http_bridge.py`（FastMCP streamable-HTTP 接口，用 `mcp`
...
依赖单一来源是 `requirements.txt`（`mcp` + `uvicorn` + `typing_extensions`，全部 `==` 固定；
