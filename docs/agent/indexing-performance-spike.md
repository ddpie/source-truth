...
|---|---|
| **日期** | 2026-06-16 |
| **关联** | source-truth MVP；MVP 待验证点「首次全量索引耗时 / 检索延迟」 |
...
本类工程的典型痛点是「未指定目录全量扫描慢，实测 **50 秒到 5 分钟**」。本 spike 用一个具备该特征的
...
1. **构造样本**：合成一个「体积约 15G、代码占比极小」的工程——目标形态是大体积、低代码比的客户端工程。
   ...
   - 说明：商业美术资源普遍受版权限制、开源渠道拿不到，故用可获得的美术资源复制填充体积；代码为真实可解析 C#。
...
- **痛点归因**：「50 秒到 5 分钟」= 冷缓存 + 串行 grep + 海量小文件遍历。缓存的影响约 16×
