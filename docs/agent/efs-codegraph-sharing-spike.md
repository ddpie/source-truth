...
> **结论一分为二：唯一被实测坐实的是「容器各自打开同一份只读索引文件」此路不通；正面方向（EFS 只读挂源码 + 远程引擎共享索引）由 API/文档支持但尚未端到端验证。**
...
2. 一个独立 index-service 在共享卷上建好的 **CodeGraph 索引，能否被会话容器只读复用**（容器内本地跑 CodeGraph 直读索引文件）？
...
1. **API 模型核验（问题 1）**：解包 boto3/botocore 的 `bedrock-agentcore-control` 服务模型（gzip JSON），
   逐字段查 `CreateAgentRuntime` 的 `filesystemConfigurations`；对照仓库钉死版本与 PyPI 最新版本，定位差异；
   再 WebFetch AWS 官方 What's New / devguide / VPC 文档核对措辞（不凭记忆）。
2. **索引引擎取证（问题 2）**：识别 codegraph-server 的索引存储引擎（`file` / strings / `--info`）；
   读 `~/.codegraph` 实际布局；穷举 `--help` flag 与 env，查是否有只读 / secondary 打开模式。
3. **只读复用实测（问题 2，决定性）**：把真实 graph.db **复制**一份（不碰原数据），`chmod -R a-w` 模拟只读挂载，
...
5. **对抗式交叉验证**：对「第 1 步 FAIL」「第 2 步 FAIL」两个关键判断各派独立 agent 尝试**证伪**
   （含 clone CodeGraph 源码逐行审计、`nm`/strings 二进制符号核对、官方文档反查），再由综合判官终裁。

> ⚠️ 方法教训：问题 1 一度因只看仓库钉死的 botocore 1.42.96（旧模型无 EFS 字段）误判为「不可行」；
> 对抗验证发现 2026-05 的新能力后，**亲自下载最新 botocore 解包核验**（不可幻觉的机器事实）才翻转结论。
> **API 能力结论必须对照实时模型 / 官方文档，钉死的旧依赖会误导。**
...
> ⚠️ 留痕说明：本机钉死的 botocore **1.42.96** 实装中 union **仅** `sessionStorage`（与下表「旧版」一致，本机直接可核）；
...
|---|---|---|
| botocore 1.42.96（仓库钉死，本机可核） | 仅 `sessionStorage` | `max:1` |
...
- **源码佐证**（对抗验证 clone 源码）：codegraph-server 9 处 RocksDB open 全是读写（`open()`/`open_with_stale_lock_recovery()`）；
  ...
  库 crate 里**实现了** `open_as_secondary`（只读、不抢锁、有单测）但**是死代码、二进制零调用**。v0.18.5 即仓库 tip。
...
> RocksDB 在两者下的失败分支预期一致（写不进 = 退化），但「真实 EROFS 下行为」归入 §6 端到端待测，本判定不替代该实测。
...
故会话容器无法直接 `--connect`，传输层**须加一层 stdio→streamable-HTTP 桥**——这正是 [`architecture.md`](architecture.md) 标注的头号风险，
...
否则 slug 不匹配会另起空图），不再跨 microVM 暴露——更可控，但非凭空消失。

---

## 6. Limitations（边界与未覆盖项，诚实标注）

- **Model B 查询通路本机零次端到端验证** ⚠️待验证（最被低估的风险）：本 spike **从未跑通一次成功的 `--serve` ← `--connect` 往返查询**
  （本机最小 stdio 驱动未驱通 `--connect`，疑 framing 差异），更未验证跨 microVM 的桥。即「会话经远程引擎查到结果」这一 Model B 核心通路
  **目前纯属架构推断**（依据是 §4.4 实测到的 socket 本地性 + `--help`/源码描述），尚无端到端证据。**不要**把 §6 下文「桥未实现」误读为「只差一座桥、往返已通」——
  往返本身也未通。首次端到端验证须在「桥 spike」完成。
- **未真正部署 AgentCore Runtime** ⚠️待验证：问题 1 结论来自 API 模型 + 官方文档（无本机部署），AWS 为只读权限，
  **未端到端实测**「IAM 省略 ClientWrite 时容器内写 `/mnt/repo` 是否返回干净 EROFS」「close-to-open 下 push 后会话多久读到 / 能否读到半写态」「冷启 ENI + 挂载延迟 vs 30s 超时」。这三项触及只读边界与一致性，文档定不死，须在部署阶段补实测。
- **stdio→HTTP 桥未实现 / 未压测** ⚠️待验证：本 spike 确认了「须有桥」与传输约束，但桥的具体实现、并发、超时、流式、microVM 内可达性均未验证——这是 Model B 的头号待验证点。
...
| 问题 | 结论 |
|---|---|
| 1. 会话容器能否只读挂载客户 EFS？ | **能**（📄 API 模型 + 官方文档，**未部署实测**）。需 `networkMode=VPC`、同账号/同 VPC/AZ 重叠、只读靠 IAM 省略 `ClientWrite`、mountPath `/mnt/<单层>`、依赖 botocore ≥ 含 2026-05 模型的版本 |
| 2. 共享卷上的 CodeGraph 索引能否被会话只读复用？ | **不能**（✅实测 + 📄源码，证据最硬）。RocksDB open 即写，只读挂载写失败并静默退化为空索引重建 |
| 3. 可行的共享 + 实时同步方案？ | **Model B**（架构自洽，**通路未端到端验证**）：索引常驻 index-service（`--serve`，独占本机 EBS，file-watcher 实时增量 ✅实测），会话经 stdio→HTTP 桥远程查询；EFS 只放源码。读写共享挂载（RocksDB-over-NFS 多写者）会腐坏，否决 |

**总判断（分级校准）**：

- **索引只读复用 = 确定性否决**（✅实测，证据最硬）——这是本 spike 唯一被端到端坐实的结论。
- **源码 EFS 只读挂载 = 方向成立，go 条件性**——依赖 §6 部署阶段三项实测（EROFS 只读边界 / close-to-open 新鲜度 / 冷启挂载延迟）。
- **Model B 远程查询 = 架构自洽，go 依赖桥 spike**——查询通路本机零次端到端验证（见 §6），实时同步部分（`--serve` 自带增量）已实测。

故方向是 **go，但「肯定面」尚立于「文档 + 推断」之上**：唯一硬实测是「容器本地直读索引此路不通」。下一步把风险收敛到 **stdio→HTTP 桥** 与 **EFS 部署实测** 两个后续 spike。

---

## 8. Next Steps（后续）

- [ ] **stdio→HTTP 桥 spike**：实现并压测 CodeGraph MCP 经 streamable-HTTP 暴露给 microVM 的桥（并发 / 超时 / 流式 / 鉴权 / 可达性）。
- [ ] **AgentCore + EFS 端到端实测**（需部署授权）：验只读边界（写 `/mnt/repo` 应 EROFS）、close-to-open 新鲜度、冷启挂载延迟 vs 30s/HTTP 424。
- [ ] **依赖版本守卫**：`scripts/check-versions.sh` 加 botocore/boto3 下限（须含 `efsAccessPoint` 的 2026-05 模型）。
- [ ] **修订 [`architecture.md`](architecture.md)**：EFS 职责缩为「只放源码」；索引段改 Model B；补 VPC / IAM 只读 / 挂载失败语义三条前提。
- [ ] **新鲜度纪律**：index-service 写者 vs 会话读者的 publish / 原子切换约定（避免读到半写状态）。
...
```bash
# 问题1：核验最新 botocore 模型是否有 efsAccessPoint（不可幻觉的机器事实）
...
# 问题2：只读复用实测（在副本上，勿碰真实 ~/.codegraph）
...
最小 MCP 驱动脚本；测试用 `--serve` 进程。清理：终止测试 `--serve` 进程（勿误杀真实 `--mcp` 服务）→ 删 `/tmp` 临时目录。
