...
cc 在 10× 重复副本上全仓检索 26 轮、消耗 $1.97；source-truth 经 CodeGraph 直接定位 `PlayerDeath.cs`
/ `StartGameBehaviour.cs`，9 次工具调用 52s 完成，本地查询近乎免费。三次独立采样（3.6× / 3.8× / 5.1×）
...
codegraph 侧本地查询近乎免费（cc 每问 ~$1）。这一批两问都不算特别复杂，所以提速倍数偏低，仍 >2×。
...
模型本身的出 token 速度**——这也是为什么单次运行的数字会大幅波动、必须多轮取中位。质量上两者都正确给出
...
## 7. Opus 4.8 vs Sonnet 4.6 复测（daggerfall，同镜像同题，4 题各暖机后计时）
