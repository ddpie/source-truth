...
检索，避开了原生 cc 在 10× 重复副本上反复的慢全仓搜索与反复试探。输出更短更聚焦（cc 输出更冗长）。
> 注：测试仓含 10 份完全相同的副本，放大了 cc 全仓搜索的劣势；真实客户单副本仓上差距会小些，
...
/ `StartGameBehaviour.cs`，9 次工具调用 52s 收尾，本地查询近乎免费。三次独立采样（3.6× / 3.8× / 5.1×）
