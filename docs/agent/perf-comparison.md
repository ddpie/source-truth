...
**结论**：接上同一个 codegraph 后，**两者基本持平**。这说明 §1 测出的 2.7–5.1× 提速**主要来自 codegraph 本身**，
不是我们的封装层。我们方案的价值在产品化（飞书即用、无需各自装 cc/配 MCP/给 AWS 凭证）、只读安全边界、
非技术用户体验（卡片流式 / 结论先行 / 出处折叠）、多项目隔离与运维——**不在纯问答速度**。
...
**结论**：**Opus 明显更快（快 ~65%），不该换 Sonnet。** 根因——Sonnet 单轮推理虽快，但为搞清同样的代码
**绕了 2–3 倍的检索轮次**（29 vs 8），每轮夹一次模型往返，累计反而更慢。质量上两者都达标（都拒绝在「每级生命」
题硬编不在仓库的数值、都给准确出处），但 Opus 用更少轮次完成、更省更稳。**这与 §2（code-5x 上两者持平）不矛盾**：
复杂取证题更能拉开「轮次效率」差距，daggerfall 这批题更吃多轮探索，于是 Opus 的轮次优势显现。维持默认 **Opus 4.8**。
