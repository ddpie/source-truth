...
> **「会生长的答案卡」在飞书现成能力内成立。** 同一张卡片可从「思考中→取证中→完成」全程流式更新、不发第二条消息；结论打字机先出、依据折叠、完成后动态追加表格 / VChart 图表 / 一键追问与转研发按钮，桌面端与移动端均正常渲染。代价是若干必须在网关层处理的硬约束（频控、10 分钟窗口、回调与流式互斥、整卡更新会清空正文）。
