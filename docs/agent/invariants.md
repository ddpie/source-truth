# 不变量与真相源映射（invariants）

这份文档把 [`AGENTS.md`](../../AGENTS.md) 的「Critical constraints」展开成**可执行的不变量**：
每条写清「是什么 / 谁是真相源 / 怎么机检 / 违反了会怎样」。AI 改代码前对照这张表，别破坏其中任何一条。

> 机检入口：`./scripts/check-invariants.sh`（结构/双语/真相源）+ `./scripts/check-versions.sh`
...
- **真相源**：被索引的目标仓库（index-service 本地副本）。其次是 `agent-container/prompts/system.md`
...
- **真相源**：`agent-container/Dockerfile`（`FROM …@sha256:…`、`setup_24.x`、`claude-code@latest`）、
...
- **真相源 → 生成物**：
  ...
  | 索引 `graph.db` | 目标仓库快照 | index-service `bootstrap.sh` 建图（单写者） |
...
- **真相源**：实际目录树 + `docs/structure_{zh,en}.md`。
...
## 5. 单写者铁律（index-service graph.db）

- **不变量**：同一时刻 graph.db 只能有**一个** codegraph-server 写者。并发写 → RocksDB 0 节点损坏。
- **真相源**：`index-service/codegraph_session.py`（常驻单 worker + 进程内锁 + orphan reaper）、
...
- **机检**：无静态机检（运行期不变量）；守卫是 flock（跨进程）+ 进程内 `_restart_lock`（非阻塞 try-acquire，
  防重启风暴死锁）+ `codegraph_client._assert_spawn_allowed()` tripwire（per-call spawn 默认禁，仅测试 opt-in）。
- **违反后果**：第二写者 → graph.db 损坏 → `/health` 报 0 节点 → 全部问答失败。
  详见记忆与 review 历史；**绝不**在 index 实例上手动再跑一个 codegraph-server 写同一份图。

## 6. MVP 只读边界

- **不变量**：仅主分支、仅只读问答、不跑引擎、不写回/提交/改文件。
- **真相源**：`agent-container/agent_lib.py` 的 SDK 配置——`tools=[]`（可用性闸）+ `disallowed_tools`
...
- **真相源**：`.local/`（gitignored 的启动脚本注入 env）；部署脚本不自动建密钥（手工建）。
- **机检**：gitleaks pre-commit（规划）；`scripts/deploy-all.sh` 的 user-data 只写非敏感配置。
- **违反后果**：密钥泄露 → 安全事故。

---

相关：变更操作手册见 [`playbooks.md`](playbooks.md)；架构心智模型见 [`architecture.md`](architecture.md)；
