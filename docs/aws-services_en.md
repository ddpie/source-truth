...
| **IAM** | 3 roles + 1 instance profile + 1 service-linked role | fixed | EC2 execution role `source-truth-index-role`, AgentCore Runtime role, DAU pre-aggregation Lambda role `source-truth-dau-lambda-role`, the instance profile, and AgentCore's VPC-ENI managed role. All three roles are account-level and shared across regions, so resource ARNs in their policies must wildcard the region segment — otherwise a second-region deploy overwrites the policy and silently revokes the first region's permissions (`check-invariants.sh` guards this) |
