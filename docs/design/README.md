...
本目录是 source-truth 的**需求与架构权威依据**：需求评审纪要、POC 架构方案，外加一份组件设计概览。
当代码 / 实现文档与这里冲突时，按 [`AGENTS.md`](../../AGENTS.md) 的「代码为唯一依据」——以代码为准并标注差异；
但**做什么、为什么这么做、MVP 边界划在哪**，以本目录为准。
...
| [`architecture-overview_zh.md`](architecture-overview_zh.md) | POC 架构方案：系统长什么样、组件分工、技术选型 |
| [`agent-container_zh.md`](agent-container_zh.md) | agent-container 组件设计概览（给人读） |
| [`multi-repo-isolation_zh.md`](multi-repo-isolation_zh.md) | 多项目 / 多仓的索引与隔离设计（post-MVP，先行文档不动代码） |

## 与其他文档的关系

设计文档回答「做什么 / 为什么」，AI 面向文档回答「一次提问怎么在系统里流转 / 改代码别破坏什么」：
...
全部文档的入口地图见 [`../README.md`](../README.md)。
