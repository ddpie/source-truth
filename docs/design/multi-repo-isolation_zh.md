...
现有实现按单仓固定——一份图、一个工作目录、一把写锁。多仓会动到隔离和「一张图只有一个写进程」这两条底线，
必须按本文设计来做。
...
调用时指定仓库则只查那一个；不指定则在该项目所有仓库里**分头查、合并结果**，每条结果标明来源仓库。

> 实现现状：codegraph 结果项无 score/rank 字段，跨仓没有可排序的相关度键，因此合并**按仓库声明顺序**（仓内沿用 codegraph 自身排序），不是严格相关度排序——见 `index-service/repo_fanout.py`。
...
防 RCE 后横向读取）尚未做——互不信任的项目须分机器。服务端越界仓名拒绝 + realpath 防穿越（§4.1）已实现。
...
增量重建内存图——无重启、不会有两个进程同时写。不对外暴露端点（零新增攻击面）；webhook / push 触发为预留项。
