...
> 实现备注：会话容器不挂任何共享文件系统，代码统一经 index-service 的 HTTP 接口读取
> **代码只存在索引服务本地磁盘**，会话容器不挂任何文件系统，定位与读文件都经索引服务的 MCP-over-HTTP 接口
...
MCP-over-HTTP 接口（定位 + 读文件工具）服务给所有会话容器，会话容器本身不挂任何文件系统；每会话独占临时
