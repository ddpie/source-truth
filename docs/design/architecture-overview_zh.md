...
**为什么这么设计**：代码和索引是项目级资源，所有人查询的是同一个项目，复制 N 份既浪费存储也导致更新
不同步。对话和临时文件是个人工作状态，必须隔离。落地方式：共享代码与索引放在索引服务本地磁盘，经其
MCP-over-HTTP 接口（定位 + 读文件工具）提供给所有会话容器，会话容器本身不挂任何文件系统；每会话独占临时
文件使用 AgentCore Session Storage（`/mnt/workspace`，按会话自动分配独占空间），无需额外开发。
...
| 场景边界 | 数值模拟是否需要运行引擎逻辑 / 是否有写回配置/提交代码的诉求 |
...
| 验证项 | 关注点 | 关联 |
|-|-|-|
| CodeGraph 对项目语言栈的索引召回 | 用项目真实模块测试调用图召回率：Unity 风格 C#（事件/委托、partial、MonoBehaviour 消息函数）与 Lua 元表继承等动态模式属静态分析盲区，实际召回可能显著低于官方基准 | §3.1 |
...
| 经 HTTP 接口读文件性能 | 会话容器经索引服务 MCP-over-HTTP 接口读文件：按项目真实仓库规模实测「索引定位 + 精准读取」主路径与全仓文本检索兜底两种模式的实际延迟，确认是否需要优化 | §3.1 / §3.3 |
