...
**SDK 选择**：新项目默认 OpenAI Agents SDK；旧项目保留原选择，升级代码不会自动改成 OpenAI。
`agent.sdk` 同时控制问答和术语表，配置及迁移步骤见 [`双 SDK 配置`](dual-sdk_zh.md)。

**区域与部署记录**：安装器建议东京 `ap-northeast-1`；应选择支持 AgentCore Runtime 且能访问所选
Bedrock 模型的区域。账户资源 ID、实际项目清单、发布记录与回滚资料放在已忽略的 `.local/` 目录中。
术语表后台重建的进度以主机日志和 `.meta` 为准，不能用 Runtime READY 代替。

## 快速开始

最简部署步骤，详细说明见后续各节。前提：本机已安装 `aws` CLI 并配置好凭证；飞书应用已按[第三节](#三接入飞书)创建，取得 `App ID` / `App Secret`。机器人 `open_id` 由安装器自动获取，失败时才需要手动填写。
...
```bash
# 复制 launch-host.sh 输出的完整命令，其中包含当前仓库地址和分支。
REPO_URL=<你的仓库地址> REPO_REF=<你的分支> bash /tmp/prepare-local-host.sh
```
...
2. **部署机（Linux 或 macOS）**：以下是 [`README 中文部分`](../README.md#中文)快速开始清单的详细要求：

   - **`aws` CLI v2**——不支持 v1。
   - **`python3`**，且 **boto3 版本足够新、带 `bedrock-agentcore-control`**（配置 Runtime 用）。
     系统 Python 由包管理器维护时，使用虚拟环境：
     `python3 -m venv .venv-deploy`、`source .venv-deploy/bin/activate`，
     再执行 `python -m pip install --upgrade boto3`，部署期间保持环境激活。
   ...
   - **GNU tar**——macOS 自带的是 BSD tar，产不出可复现的归档；缺了它索引主机会把产物判定为「每次
     ...
     装出的 `gtar` 会被自动识别。
   - **[Session Manager plugin](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html)**——交互登录实例时需要，不随 AWS CLI 附带；缺失仅告警，不阻止自动部署。
   ...
   （私有仓经 `gh`，公开仓经直链）。
3. **Bedrock 模型访问**：部署身份会通过 Converse 探测所选模型；若该身份被拒，仅告警并由部署后验收核对 Runtime 身份。Runtime 与索引机角色需要所选模型的
   `bedrock:InvokeModel` / `bedrock:InvokeModelWithResponseStream`。OpenAI 问答与术语表均使用
   `ConverseStream`；标准 IAM 阶段会配置运行角色权限。
   部署查询 `bedrock list-inference-profiles`，只解析同一模型可用的 system inference profile 前缀，
   不会换用其他模型；例如 Claude 在东京可解析为 `jp.…`，OpenAI 使用对应的 `global.openai.…`。
   ACTIVE 列表或预检通过均不替代真实调用验证。
...
先准备好飞书应用（第三节），拿到 `App ID` / `App Secret`；机器人 `open_id` 由安装器自动获取。
...
公开仓库不要求 GitHub 登录。使用私有 fork 时，先 `gh auth login`，再设置
`SOURCE_TRUTH_SLUG=owner/repo`；覆盖参数见 [`scripts/README.md`](../scripts/README.md)。

`install.sh` 是一个交互菜单（键盘上下键选择、回车确认），四个流程见 [第七节 多项目](#七多项目一台机器多个机器人)。
默认选中「添加项目」，不必先初始化环境。新环境的术语表与监控默认关闭；旧环境保留原选择。
首次部署的典型顺序：

1. **查依赖**：`aws` / `python3` / `docker`（含守护进程在运行）/ `git`，可选 `gh`（私有仓部署需要），并校验 AWS 凭证可用；
2. **选「添加项目」**（底座不存在会自动先建）：填 projectId → 逐个加仓库（**git 地址** + 子目录 + 分支）→
   索引服务端口（即该项目的 `index-bridge-<项目>` 进程监听端口，脚本自动建议）→
   Agent SDK（默认 OpenAI）及回答模型（术语表沿用该模型） → 飞书 App 凭证（自动写入
...
**可选能力**：添加项目时选择是否启用术语表和监控，也可预先传入 `--with-glossary` / `--with-monitoring`。
术语表开启后，默认每仓扫描上限为 **400 文件**；`--glossary-max-files 0` 才表示不限量。
问答与术语表使用同一 SDK，首次安装沿用回答模型；高级配置可单独设置 `agent.glossaryModel`。
关闭术语表会停止后续生成，git 刷新继续，已生成的有效词条仍可查询。

| 上限 | 用途 |
|---|---|
| `400`（开启后的默认） | 首次体验，限制模型调用量 |
| `1000` / `4000` | 扩大覆盖范围 |
| `0`（显式选择） | 扫描全部候选文件，费用随仓库规模增长 |

配置不变时通常按代码变更增量构建；切换 SDK / 模型会全量重建。纯英文文件也可能产生调用费用。
`--with-glossary=false` 停止术语表生成；`--with-monitoring=false` 跳过后续监控部署，
已经创建的告警、定时任务等仍运行。后续运行会沿用已保存的选择。
旧部署没有这些开关时按原先已开启处理，不会因升级默认值而停用。

> 术语表为**后台异步**构建：部署完成后问答功能立即可用；大仓首次全量构建可能耗时数十分钟，其间问答
> 功能不受影响，仅中文冷僻词可能尚未对应。构建进度与结果记录在主机日志中（`journalctl` 查
> `glossary_gen_done` / `glossary_gen_cc_failed`，后者也可能来自 OpenAI）。
> 构建失败时保留旧表，首次构建失败则暂无术语表；问答仍可直接检索代码。
> SDK 切换还需核对新产物 `.meta` 与当前配置匹配，具体见
> [`术语表切换`](dual-sdk_zh.md#术语表切换与运维)。
...
首次添加项目仍交互填写凭证；脚本化首建按附录 A 准备项目清单与密钥后运行 `deploy-all.sh`。
每份 `.local/deploy-config` 只对应一个区域，区域不匹配会提前拒绝；另一地区使用独立检出目录。
部署后自动运行一个通用代码问答（模型调用会计费）。退出码 `3` 表示服务已部署但验收未通过，
也包括探针无法运行、返回 `2` 的情况；
`--no-probe` 可跳过，纯本地仓在上传代码前自动跳过。
...
- **当前部署拓扑须保留 NAT**：实例位于公有子网（有公网 IP 供 SSH），AgentCore Runtime 位于私有子网，经 NAT 访问 Bedrock。Runtime 网卡没有公网 IP，仅配 IGW 路由无法访问公网。脚本创建 ECR 与 S3 VPC 端点用于拉镜像，未配置 Bedrock Runtime 端点。NAT、公网 IPv4 和接口端点均有区域相关费用，详见[服务清单](aws-services_zh.md)。bridge 端口（8080-8099）仅对同一安全组内成员开放。
...
2. **启用机器人**：在「机器人」页打开机器人能力。安装器使用凭证调用 `/open-apis/bot/v3/info`
   自动取得 `FEISHU_BOT_OPEN_ID`，失败时才提示输入；不需要在控制台复制该值。
   **须先启用机器人，后续的发消息权限才能生效。**
...
- **凭证交给安装器**：`App Secret` 是敏感信息，**绝不入库**。第二节的 `install.sh`「添加项目」会问
  `App ID` / `App Secret`，自动取得机器人 `open_id` 后写进 **Secrets Manager**（密钥名按项目区分
...
**停用旧区域网关**：在旧主机上执行
`sudo systemctl disable --now bot-gateway@<项目>.service`，并用
`sudo systemctl show bot-gateway@<项目>.service -p ActiveState -p UnitFileState -p MainPID`
确认 inactive、disabled、MainPID=0。这只停网关，EC2、Runtime 等资源仍保留。
后续不要对已停用项目运行会重新 enable/restart 网关的部署命令。
...
1. **离线套件 + 真实端到端探针**（在部署机上，不用进实例）：
   ...
   出处。部署使用的 `--smoke` 还必须看到成功的 `codegraph_read_file` / `codegraph_read_table` 完成事件；拒答或仅写文件名不算通过。规范化流混入未知格式、缺终态或报告错误均失败。`2` 视为 skip 而非失败——离线或尚未部署的环境跑 `--full` 不会被它阻塞。
...
4. **在群里 @ 机器人**并提问（如「装备耐久怎么算？」）。预期：
   ...
   - 正常、有代码依据的回答末尾应显示「继续追问」和 2–3 个问题按钮；简短回答也要检查。仅出现“回复本条消息”的提示不算列表验收通过。点击问题或直接回复卡片后，应延续上文继续提问。
...
**改术语表开关或上限**：使用 `install.sh --action redeploy --project <项目> --with-glossary --glossary-max-files <n>`。
开关和上限已纳入部署签名；即使代码未变化，也会重新下发到主机，并刷新同机其他项目的配置。
安装器用 `.local/deploy-config` 的 `DEPLOY_GLOSSARY_PENDING_PROJECTS` 持久记录未下发成功的项目，
重跑时继续处理；配置已应用但问答验收返回 `3` 的项目不重复下发。
成功后才记录已应用的值，失败重跑会继续尝试。当前实现会就地重跑 bootstrap，网关有数分钟中断；
实例、仓库和索引保留。无需手工修改 `/etc/index-service.env`。

**只重部署 runtime**（修改 agent 镜像 / system prompt 后）：重新运行 `deploy-all.sh`（镜像与 runtime 阶段幂等）。
已有 microVM 在回收前仍使用旧镜像。网关重启会清除缓存的会话 ID，后续问题用更新后的 Runtime 新建会话；正在处理的请求可能仍用旧版本。
...
`followup_suggestions`（`count` 为提取的问题数，`markerPresent` 区分缺少标题与解析不到问题；不记录问题正文）、
...
```bash
./scripts/trace.sh st-0123456789abcdef0123456789abcdef   # 示例值，把它换成卡片底部或 answer_* 日志行里的真实 traceId
...
**监控：指标 / 看板 / 告警**——启用 `--with-monitoring` 后，`deploy-all.sh` 的 Phase 7 部署整套（CloudWatch 指标 filter、三页看板、告警 + SNS、DAU 预聚合 Lambda），**正常无需手动干预**。例外是**首次部署**：网关还没写过日志、log group 尚不存在时，指标 filter 与告警会建不出来（deploy 只 WARN 不失败）——在群里提一个问题后重跑一次 deploy（或[附录 D](#附录-d手动刷新监控) 的命令）即补齐。此外只在单独刷新看板/阈值、或 deploy 时 `--skip monitoring` 后需要补充执行时才手动运行。告警的 SNS 订阅需手动确认一次（邮件点确认链接）。
...
**拆除部署资源**：试用完、或某次部署中途失败留下计费资源（NAT、公网 IPv4、EC2、两个 ECR 接口端点）时，一条命令按反依赖顺序清理：
...
`agent`（SDK、回答模型、术语表模型，见 [配置与迁移](dual-sdk_zh.md)）、
...
| 症状 | 可能原因 | 处置 |
|------|----------|------|
| 部署显示成功，但每次问答都答不出内容（空答案 / 「未找到」） | **Runtime 到 bridge 的这一段可能不通**——先看部署的真实问答验收结果；使用 `--no-probe` 或本地仓尚未上传导致探针跳过时，这段链路未验证 | 按这三处依次查：① runtime 使用的安全组是否放行到索引主机的 **8080-8099**；② 私有子网到 **NAT** 的路由是否存在（Runtime 网卡由 AWS 托管、无公网 IP，必须经 NAT）；③ 私有域名能否解析——进实例跑 `dig +short index.<r>.source-truth.internal`，返回空说明私有托管区 / VPC DNS 属性有问题（见[附录 C](#附录-c首次部署后的真机核对清单)） |
...
| 卡片回「查询失败」/ 日志 `AccessDenied` | 实际调用的 Runtime / 索引机角色缺模型权限，或模型档在该区域不可用 | 先定位报错角色，再补所选模型及推理档的调用权限；部署身份能调用不代表 Runtime 能调用。见前置条件 3 与 [SDK 配置](dual-sdk_zh.md) |
...
| 重新部署后行为仍是旧版本 | 仍存活的 microVM 继续使用旧镜像 / 网关未重启 | 等待该 microVM 回收；重启网关以确保运行新代码 |
| 中文问答未用上项目专属命名 / 术语表疑似为空 | 术语表后台构建未完成或失败（所选 SDK 依赖缺失 / Bedrock 不可达或无权限） | 进实例查看 `journalctl` 与 `/var/log/glossary-build-*`，查 `glossary_gen_done`（成功）/ `glossary_gen_cc_failed`（构建失败）；不影响问答，问答会自动退回常规检索 |
...
- **越界能力后置**：多分支、设计文档读取、写回、第二引擎等均为 post-MVP，详见
  [`README 的范围与限制`](../README.md#范围与限制)与设计权威依据 [`design/`](design/)。
...
| `--with-glossary[=true\|false]` | 新环境关闭 | 开启或关闭后台术语表生成；旧部署保留原选择 |
| `--glossary-max-files <n>` | `400` | 开启术语表后的每仓文件上限；`0` 表示不限量。变更会自动重新下发 |
| `--with-monitoring[=true\|false]` | 新环境关闭 | 开启监控部署；关闭开关不会删除已创建的监控资源 |
| `--no-probe` | 不跳过 | 跳过部署后的真实代码问答验收 |
...
> **真机上最易出问题的两处，交付前务必亲自确认**：一是「部署显示成功、问答却答不出内容」——原因几乎总在 Runtime 到 bridge 这一段（安全组未放行 8080-8099 / 私有子网到 NAT 的路由不通 / 私有域名解析失败），默认 Runtime 探针会验证这段链路，跳过探针时需补验；二是安全组 22 端口放行的出口 IP 不对——无法登录刚创建的机器。

---

## 附录 D：手动刷新监控

启用 `--with-monitoring` 后，`deploy-all.sh` 的 Phase 7 部署整套监控；以下命令也可用于显式安装监控、单独刷新看板/阈值，或在 `--skip monitoring` 后补充执行。部署期身份需 `logs:PutMetricFilter`、`cloudwatch:PutDashboard`、`cloudwatch:PutMetricAlarm`、`sns:CreateTopic`（非运行时角色）。幂等、可重复运行，换区域改 `--region` 即可。
...
启用监控时，它也是 `deploy-all.sh` Phase 7 的一部分。每一步做完都回读校验，而不是以退出码为准——
...
到实际配置的 trace 目的地检查 span。OpenAI 的 SDK scope 为
`openinference.instrumentation.openai_agents`，Claude 为
`openinference.instrumentation.claude_agent_sdk`；此外可能有传输层与 Runtime span。
OpenAI 项目不应以是否存在 Claude subprocess span 判断健康。
...
`logs:PutResourcePolicy`。部署脚本不自动启用此 Runtime 环境变量；
检查已部署环境后再选择要查询的日志组。
...
评估是**可选**的，不在部署必经路径上：机器人回答问题不需要它，它花的也是另一类钱（Lambda 调用与运行时长，
...
按项目 SDK 区分：Claude 的 `openinference-instrumentation-claude-agent-sdk` 自动加载；
OpenAI 的 `openinference-instrumentation-openai-agents` 由 `openai_backend.instrument()`
显式安装独占 processor，两者均经 ADOT 导出。到实际 span 目的地检查
（默认 `aws/spans`；启用 unified trace destination 后可为该 Runtime 日志组）：
...
未查到匹配 span 时，先核对时间范围、span 目的地及所选 SDK 的插桩，再配置评估。
查错日志组得到 0，不代表插桩失效。
...
阶段依次是 `package` → `iam` → `lambda` → `evaluators` → `online`，每步幂等；`--dry-run` 无 AWS 调用。
更新 Lambda 时保留额外环境变量。
新实时评估配置默认禁用、采样 100%；`--enable --sampling 20` 启用并设置采样百分比。
已有配置未显式传启停或采样参数时保留原值；`--disable` 显式停用。
重跑会核对并更新启停、采样率、评估器列表、会话超时和执行角色；读不到已有配置时返回失败。
评委模型按当前区域解析，也可用 `EVAL_JUDGE_MODEL=<完整推理档 ID>` 指定；它独立于问答与术语表的 SDK 选择。
新建角色的传播延迟会有限重试，权限错误仍会失败。目标区域必须与本地部署配置一致。

**代码型评估当前只支持一个项目的 bridge。** `--project` 选择 Lambda 回查的项目。新建时缺省取第一个项目，更新时沿用已有 bridge 对应项目；无法唯一匹配时要求明确指定。
多仓库前缀按该项目生成并通过 JSON 传递。`online` 仍为区域内各 Runtime 建配置，
因此不能把同一个代码型评估器用于其他项目后，将其结果当作有效评分。多项目自动路由尚未实现。

几点值得知道：

- **打包必须在容器里做。** `pydantic` 带 `pydantic-core` 二进制轮子，用本机 pip 装出来的包在 Lambda 上
  可能直接 import 失败，而那种失败只在真正评估时才暴露、并且会以「评估器故障」的形式出现在评估数据里。
  脚本用 Lambda 官方基础镜像，以 `linux/arm64` 构建并验证 import；x86 部署机须支持 ARM64 仿真。
