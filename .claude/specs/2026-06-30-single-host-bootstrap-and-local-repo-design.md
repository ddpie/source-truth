...
> **实现已演进（2026-06-30，以现状为准）**：本节原设计的「推完停 bridge → 全量重建 → 失败回滚」在实现时
> 应需求改为**增量更新**，与 git 仓的 watcher 增量同一条路径（本节当初把增量列为 DEFERRED 优化，后取消该
> 延后）。最终行为：① push 先把代码经网络 rsync 到暂存目录 `/data/repo/<subdir>.incoming`（不碰正在服务的
> 代码）；② host 侧用 `rsync --delay-updates` 原地同步到 `/data/repo/<subdir>`，常驻 codegraph 的 file-watcher
> 几秒内增量重建索引、**不停 bridge**；③ 术语表按本次同步的变更文件清单（rsync `--itemize-changes` 导出）走
> 增量重建（喂 `glossary_gen --changed-list/--deleted-list`）。**唯一例外**：首次推送（尚无 graph）仍停 bridge
> 全量建一次图。`reindex_local_repo.sh` 会校验目标确为 `source:"local"`，对 git 仓 fail-loud。中断处理见 §7。
> 取舍记录：增量是「不停机 + 术语表省 token」对「+~137 行复杂度 + 一个被 `--delay-updates` 收窄的中途一致性
> 窗口」，针对预期会反复推送的本地仓场景；若推送极低频，§4.3 原方案（全量重建）更简单。
