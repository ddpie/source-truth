...
const STANDALONE_PREAMBLE_OPENERS: RegExp[] = [
  ...
  new RegExp(`^.{0,48}(可以|现在|来|开始|这就)(作答|回答|给出答案|给出结论|下结论)(了|啦)?${TAIL}$`),
  ...
  new RegExp(`^.{0,60}[，,。]?(现在|来|这就|接下来|下面|直接)(就)?(我)?(来|开始|直接)?(分类|逐类|逐一|依次|分别)?(整理|汇总|给出|得出|呈现|输出|回答|说明|讲解|讲讲|讲|说)(成|出)?(一下)?(完整|最终|对比)?(的)?(答案|结论|回答|内容|信息|结果|输出)?${TAIL}$`),
