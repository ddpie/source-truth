...
/** After close streaming: append a footer separator. The recommended follow-up
 *  questions are generated by the Agent itself (in the answer text, per system
 *  prompt instruction), so no extra API call is needed for that content. */
export async function appendFooter(cardId: string, sequence: number): Promise<void> {
  const elements = [
    { tag: "hr" },
    { tag: "markdown", content: "💡 复制上面的追问直接发送即可，上下文会延续。" },
  ];
