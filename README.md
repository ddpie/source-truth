...
**index-service**（CodeGraph 索引 + 文件读取桥）定位并读取最新主分支源码与配置表，再以 **CardKit 流式卡片**
...
策划 ──@助手──▶ 飞书 ──长连接事件──▶ bot-gateway (TS 长驻网关)
                          ...
                          ┌──────────────────────┴────────────────────┐
                          ▼                                             ▼
              index-service MCP-over-HTTP（常驻）              Session Storage /mnt/workspace
              CodeGraph 定位 + 文件读取（read/glob/search）      (per-session 临时文件)
              最新主分支代码+配置表（本地副本，HTTP 桥服务）
                          ▲
...
CodeGraph 类索引、index-service 持仓库本地副本经 HTTP 桥服务代码、每游戏项目一个机器人、机器人内按会话隔离。设计文档读取、多分支
...
# 一键部署（全新账号/区域可跑，幂等）：artifacts→IAM→network→index-service→镜像→Runtime
./scripts/deploy-all.sh --region <region> --repo <local-repo-path>   # 加 --dry-run 仅打印计划
# deploy.sh 已废弃，仅作兼容垫片转发到 deploy-all.sh
```

> ⚠️ MVP 阶段基础设施先用 `agentcore` starter toolkit + boto3 起步跑通主流程与 POC，待 CodeGraph 召回率 /
> 增量索引 inotify 等待验证点验证后再渐进 CDK 化。
