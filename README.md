...
部署在单一账号、单一区域（默认东京 `ap-northeast-1`）。下表中 **N = 项目数**（每个游戏项目一个机器人，
按项目独立的资源会随项目数增长）；其余为全局共享。规格均为默认值，部署时可调。
...
| **NAT Gateway**（+ 弹性 IP） | 1 个，置于公有子网 | 1 | 私有子网出站（拉取 S3 产物、调用 Bedrock） |
...
> （无 EFS），源码全经 index-service 的 HTTP 接口读取。
